...
export default function MainLayout() {
    ...
    <div className="space-y-3">
      ...
      <div className={`rounded-lg border ${c.sidebarBorder} ${c.sidebarBg} p-3 space-y-2 relative z-50`}>
        ...
        <input
          ...
          className={`w-full px-2 py-2 text-xs border ${c.sidebarItemBorder} rounded-lg ${c.sidebarItemBg} ${c.sidebarItemText}`}
        />
      </div>
        ...
        <button
          ...
          className={`w-full text-left px-3 py-3 rounded-lg transition-colors flex items-center gap-2.5 ${
            selectedView === id
              ...
              : `${c.sidebarItemBg} ${c.sidebarItemText} border ${c.sidebarItemBorder} ${c.sidebarItemHover}`
          }`}
        >
      ...
      <div className={`${c.sidebarBg} rounded-lg border ${c.sidebarBorder}`}>
        ...
        {coursesExpanded && (!isSidebarCollapsed || isMobileMenuOpen) && (
          <div className={`border-t ${c.sidebarBorder} p-1.5 space-y-1`}>
            {courses.length === 0 ? (
              <div className={`px-3 py-3 text-center text-xs ${c.mutedText}`}>No courses found</div>
            ) : (
              courses.map((course) => (
                <button
                  key={course.id}
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-md text-xs font-medium border border-transparent ${
                    selectedCourse?.id === course.id && selectedView === 'courses'
                      ? `${c.sidebarSelectedBg} ${c.sidebarSelectedText} border-${c.sidebarSelectedBorder}`
                      : `${c.sidebarItemText} ${c.sidebarItemHover}`
                  }`}
                >
  ...
  return (
    <div className={`h-screen flex flex-col overflow-hidden ${c.bg}`}>
      {/* Top Header Bar */}
      <header className={`${c.sidebarBg} border-b ${c.sidebarBorder} px-3 md:px-6 py-3 flex items-center justify-between z-30`}>
        {/* LEFT: Brand */}
        ...
        {/* MIDDLE: Action Buttons with Pill Styling */}
        <div className="flex gap-3 items-center">
          <button
            onClick={() => setIsCourseModalOpen(true)}
            className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${c.sidebarItemBg} ${c.sidebarItemText} ${c.sidebarItemBorder} ${c.sidebarItemHover}`}
          >
            <GraduationCap className={`w-4 h-4 ${c.moduleIcon}`} />
            <span className="hidden md:inline">New Course</span>
          </button>

          <button
            onClick={() => setIsAddActivityModalOpen(true)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${c.sidebarItemBg} ${c.sidebarItemText} ${c.sidebarItemBorder} ${c.sidebarItemHover}`}
          >
            <Plus className={`w-4 h-4 ${c.moduleIcon}`} />
        ...
        <div className="flex gap-1.5 items-center md:gap-3">
          ...
          {kids && kids.length > 0 && (
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1 px-2 md:px-3 py-1.5 border ${c.sidebarItemBorder} ${c.sidebarItemBg} rounded-lg text-xs md:text-sm`}
              >
              ...
              {showKidDropdown && (
                <>
                  ...
                  <div className={`absolute right-0 mt-2 w-48 ${c.cardBg} rounded-lg shadow-lg border ${c.sidebarBorder} py-1 z-40`}>
                    {kids.map((kid) => (
                      <button
                        key={kid.id}
                        onClick={() => {
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-1.5 px-2 md:px-3 py-1.5 border ${c.sidebarItemBorder} ${c.sidebarItemBg} rounded-lg text-xs md:text-sm`}
            >
            ...
            {showYearDropdown && (
              <>
                ...
                <div className={`absolute right-0 mt-2 w-36 ${c.cardBg} rounded-lg shadow-lg border ${c.sidebarBorder} py-1 z-40`}>
          ...
          <div className="hidden relative sm:block">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 border ${c.sidebarItemBorder} ${c.sidebarItemBg} rounded-lg text-sm`}
            >
            ...
            {showThemeDropdown && (
              <>
                ...
                <div className={`absolute right-0 mt-2 w-52 ${c.cardBg} rounded-lg shadow-lg border ${c.sidebarBorder} py-2 z-40`}>
                  {Object.entries(themes).map(([key, themeData]) => (
                    <button
                      ...
                      className={`px-3 py-2 text-left text-sm w-full ${c.sidebarItemHover} ${c.text}`}
                    >
                      {themeData.name}
                    </button>
                  ))}
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          <div className={`border-t ${c.sidebarBorder} flex items-center justify-between md:hidden pt-4`}>
            <button onClick={signOut} className={`flex font-semibold gap-2 items-center p-2 text-xs ${c.mutedText}`}>
              <LogOut className="h-4 w-4" /> Sign Out
            </button>
          </div>
        </aside>

        {/* View Main Content Background Matched to Theme */}
        <main className={`${c.bg} flex-1 md:pb-0 overflow-y-auto pb-16`}>
        ...
        />
            ...
            <AnalyticsDashboard kidId={selectedKid.id} />
          ) : selectedView === 'settings' ? (
            <SettingsView selectedSchoolYear={selectedSchoolYear} selectedKid={selectedKid} />
          ) : (
            <div className="flex h-full items-center justify-center p-6 text-center">
              <BookOpen className={`h-12 mb-2 w-12 ${c.mutedText}`} />
              <p className={`font-medium ${c.mutedText}`}>Select a view from the menu to get started</p>
            </div>
          )}
        </main>
      </div>
      ...
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 ${c.sidebarBg} border-t ${c.sidebarBorder} flex justify-around p-2 z-30`}>
        {[
          { id: 'agenda', label: 'Agenda', Icon: Home },
          { id: 'calendar', label: 'Calendar', Icon: Calendar },
          { id: 'planner', label: 'Planner', Icon: LayoutGrid },
      ...
      {selectedKid && isAddActivityModalOpen && (
        <ActivityCreateModal
          kidId={selectedKid.id}
